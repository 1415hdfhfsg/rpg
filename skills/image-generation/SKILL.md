...
5. **그림자 테스트**: "그림자 방향이 광원과 일치하는가? 그림자 색이 맞는가?"
   - 그림자는 절대 순수 검정이 아님 → 환경색 + 광원 보색 반영
   - 접촉 그림자(물체가 바닥에 닿는 곳)가 가장 어두움
   - 광원이 확산광(흐린날/형광등)이면 그림자 거의 없어야 함
6. **색채 테스트**: "이 광원의 색온도가 피사체 색에 영향을 줬는가?"
   - 형광등(4000K) 아래 금색 → 약간 탁하고 차가워진 금색
   - 텅스텐(2700K) 아래 흰색 → 노란 끼
   - 혼합 광원이면 색 충돌이 반영되었는가?
7. **대비 테스트**: "명암 대비 수준이 조명 조건에 맞는가?"
   - 형광등 = 낮은 대비, 직사광 = 높은 대비, 흐린날 = 매우 낮은 대비

**Full references** (Claude MUST read before first image generation):
- `references/visual-physics.md` — 그림자 형성 원리, 재질별 빛 반응, 색온도표, 혼합광원, 그림자 색 규칙
- `references/instagram-photography.md` — 장면별 촬영 패턴(구도/조명/색감/소품), 5가지 컬러 그레이딩 프리셋, 흔한 실수 목록, 장면→프리셋 매핑표
